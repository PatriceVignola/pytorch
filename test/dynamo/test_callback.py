...
class CallbackTests(TestCase):
    ...
    def test_callbacks_without_duplicate_prevention(self) -> None:
        callback_handler._CompilationCallbackHandler__prevent_duplicate_callbacks = False
    ...
    def test_counter(self) -> None:
        ...
        with callback_handler.install_callbacks():
            self.assertEqual(callback_handler._CompilationCallbackHandler__pending_callbacks_counter, 1)
        self.assertEqual(callback_handler._CompilationCallbackHandler__pending_callbacks_counter, 0)
    
    def test_counter_assertion(self) -> None:
        callback_handler._CompilationCallbackHandler__prevent_duplicate_callbacks = True
        callback_handler._CompilationCallbackHandler__pending_callbacks_counter -= 1

        with self.assertRaises(AssertionError) as e, callback_handler.install_callbacks():
            pass
